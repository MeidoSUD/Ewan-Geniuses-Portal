...
export const WalletTab: React.FC<WalletTabProps> = ({ user }) => {
  ...
  const loadData = async () => {
      // Fetch Banks List
      try {
          const banksList = await referenceService.getBanks();
      ...
      try {
          const methods = await teacherService.getPaymentMethods();
          setMyBankAccounts(methods);
      } catch (e) { 
          console.error("Failed to load payment methods:", e); 
      }

      // Fetch Wallet (Isolated failure handling)
      try {
          const wallet = await teacherService.getWallet();
          setWalletData(wallet);
      } catch (e) { 
          console.error("Failed to load wallet data (Backend Error):", e);
          // Optional: Set dummy data or leave null to show user's profile balance as fallback if available
      } finally {
  ...
  const handleWithdraw = async () => {
      ...
      try {
          ...
          console.error(e);
          alert("Failed to request withdraw. Check your balance.");
      } finally {
          setLoading(false);
      }
  };
  ...
  // Fallback balance from user profile if wallet API fails
  const displayBalance = walletData?.balance ?? user?.profile?.current_balance ?? 0;
  const displayWithdrawals = walletData?.withdrawals?.data || [];
        ...
        <div className="md:col-span-2 bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-4xl font-bold mb-6">
              {displayBalance} <span className="text-lg font-normal text-slate-400">{t.sar}</span>
            </h2>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col justify-center">
            ...
            <div className="text-2xl font-bold text-slate-900 mb-2">
               {displayWithdrawals
                   ...
                   .reduce((sum, w) => sum + Number(w.amount), 0).toFixed(2)} {t.sar}
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {myBankAccounts.map((acc) => {
             // Determine Bank Name (from nested object or fallback)
             const bankName = language === 'ar' 
                ? (acc.banks?.name_ar || "Bank") 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {displayWithdrawals.map((tx) => (
                  <tr key={tx.id} className="hover:bg-slate-50/50">
                    <td className="px-6 py-4 text-slate-600">
                        {new Date(tx.requested_at).toLocaleDateString()}
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-900">
                        Withdrawal to {language === 'ar' ? tx.payment_method?.banks?.name_ar : tx.payment_method?.banks?.name_en}
                    </td>
      ...
      <Modal isOpen={showWithdraw} onClose={() => setShowWithdraw(false)} title={t.requestPayout}>
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg mb-4">
            <p className="text-sm text-blue-700">Available Balance: <strong>{displayBalance} {t.sar}</strong></p>
          </div>
          ...
          <Select 
            ...
            options={[
                ...
                ...myBankAccounts.map(acc => ({ 
                    value: String(acc.id), 
                    label: `${language === 'ar' ? acc.banks?.name_ar : acc.banks?.name_en} (****${acc.account_number.slice(-4)})`
                }))
            ]}
